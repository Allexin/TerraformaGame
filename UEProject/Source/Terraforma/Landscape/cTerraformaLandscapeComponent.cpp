	...
	for (int i = 0; i < width*height; i++) {
		...
		if (i >= MaterialsReadyCount || m_DynMaterialInstances[i]->Parent!= baseMaterial) {
			...
			if (i >= MaterialsReadyCount) {
				m_HeightmapTextures[i] = UTexture2D::CreateTransient(CHUNK_H_RESOLUTION + 4, CHUNK_H_RESOLUTION + 4, PF_R8G8);// PF_R16_UINT);
				...
				m_NormalmapTextures[i] = UTexture2D::CreateTransient(CHUNK_H_RESOLUTION + 4, CHUNK_H_RESOLUTION + 4, PF_R8G8B8A8);
				...
				m_ColorTextures[i] = UTexture2D::CreateTransient(CHUNK_T_RESOLUTION + 4, CHUNK_T_RESOLUTION + 4, PF_R8G8B8A8);
		...
		FMemory::Memcpy(DataH, chunk->dynDataHeightMap, (CHUNK_H_RESOLUTION + 4)* (CHUNK_H_RESOLUTION + 4) * sizeof(sChunkHeightData));
		...
		FMemory::Memcpy(DataN, chunk->dynDataNormalMap, (CHUNK_H_RESOLUTION + 4)* (CHUNK_H_RESOLUTION + 4) * sizeof(sChunkNormalData));
		...
		FMemory::Memcpy(DataT, chunk->dynDataTexture, (CHUNK_T_RESOLUTION + 4)* (CHUNK_T_RESOLUTION + 4) * sizeof(sChunkTextureData));
			...
			for (int y = 0; y < m_Landscape.height(); y++)
				for (int x = 0; x < m_Landscape.width(); x++) {
					...
					if (chunk->TextureChanged) {
						...
						FMemory::Memcpy(Data, chunk->dynDataTexture, (CHUNK_T_RESOLUTION + 4)* (CHUNK_T_RESOLUTION + 4) * 4);
						Mip.BulkData.Unlock();
						m_ColorTextures[i]->UpdateResource();
					}
